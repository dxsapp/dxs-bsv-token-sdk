...
- **2026-04-18** — **Phase 1 Step A.1.0 pseudo-ASM audit** (`BNTP_V2_NORMAL_TEMPLATE_AUDIT.md`, verdict **audit-PIVOT**): assistant ran adversarial review of opus-agent pseudo-ASM before committing to real ASM writing. Found **3 critical spec-level gaps** + moderate byte-count uncertainties (±100b per section). User ratified closure of 5 new decisions (#27-#31, spec §15):
  - (27) Normal↔Frozen cross-template: **hash + unlocking push** for rare paths (freeze pushes Frozen body, unfreeze pushes Normal body; SHA256 match verified on-chain). §5.5.1.
  - (28) Contract→Normal cross-template: **inline Normal body as constant** in Contract body (one-shot mint amortization). Contract body grows 500b → ~3100b; issue unlocking stays ~500b instead of `N × 2500b`. §5.5.2.
  - (29) Explicit PKH owner CHECKSIGVERIFY: mandatory, independent from covenant CHECKSIG (which is preimage-auth only). +~5b PREFIX. §8.1.
  - (30) Explicit `M` push in flex-transfer unlocking: avoids fragile OP_DEPTH derivation. +~3b body. §9.2.
  - (31) BSV post-Genesis consensus normative: Normal body exceeds pre-Genesis opcode/stack limits; SDK gates deploys to compliant networks. §13.6.
