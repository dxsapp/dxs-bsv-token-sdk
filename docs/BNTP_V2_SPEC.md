...
**Cross-document references.** This spec cites some §3.x subsections (e.g., §3.2 covenant, §3.3 sighash check, §3.6 owner identity) that live in the **pseudo-ASM companion document** `BNTP_V2_TEMPLATE_NORMAL_ASM.md`, not in this spec. Those references describe the implementation-level structure of the Normal template body and are normative for ASM writers but intentionally out-of-line from this spec's protocol-level rules. Real-ASM artifacts are in `BNTP_V2_NORMAL_TEMPLATE_A1_1_REPORT.md`, `A2_REPORT.md`, `A2_5_REPORT.md`. Readers of this spec alone may safely ignore §3.x cross-refs; readers implementing the ASM must consult the companion docs (Step C Minor #2 editorial fix).
...
| --- | ---------- | ----------- | ----------------------------------------------------- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0   | `Contract` | `OP_0`      | Pre-issuance reserve                                  | Normal × N (issue)    | **3971b measured** (Phase 1A.3, decision #49; inlines Normal body 2620b + shared PREFIX 736b + path-6 logic ~615b; gate revised PASS ≤4100 / PIVOT ≤4300 / ABORT >4300; one-shot UTXO, cost amortized over token lifecycle) |
| 1   | `Normal`   | `OP_0`      | Spendable token, all owner + authority + issuer paths | Normal, Frozen, P2PKH | **2620b measured** (Phase 1A.1 final, §11.1; gate PASS ≤2600 / PIVOT ≤2700 / ABORT >2700; PIVOT accepted per Step C Critical #1 disposition)                                                                                |
| 2   | `Frozen`   | `OP_2`      | Frozen token                                          | Normal                | **1282b measured** (Phase 1A.3, gate revised PASS ≤1200 / PIVOT ≤1400 / ABORT >1400; PIVOT, spec §11.1 pre-audit 700b projection stale)                                                                                     |
...
Normal
  ...
  └─(confiscate, confiscAuth sig)───► Normal × 1         [new owner, depth PRESERVED (= my_depth)]
...
4. **Confiscation preserves depth (Step C Critical #1 fix).** Confiscation is a pure ownership change — the confiscAuth signs off on who owns the UTXO, not on its freshness. Previous v2 drafts reset depth to 0 on confiscate with the rationale "authority vetted the new owner"; that design conflated the confiscAuth trust domain with the issuer trust domain. A compromised or adversarial confiscAuth could mass-confiscate to attacker-chosen owners at depth=0, producing tokens indistinguishable (from wallet perspective) from issuer-attested refreshes — a silent fresh-token forgery surface cascading from confiscAuth compromise. Under preserve-depth semantics, the new owner inherits whatever depth the prior holder had; they may call `refresh` to legitimately reset depth via issuer attestation if desired. Symmetric with `freeze`/`unfreeze` (rules 5, 6).
...
11. **Amount sum conservation enforced on every spending path that produces BNTP outputs (Step C Moderate #2 fix).** No burn path exists in v2. Flex-transfer: `Σ input_amounts == Σ output_amounts`. Refresh: `my_amount == refreshed.amount + royalty.amount`. Freeze/confiscate: output.amount == input.amount (1:1 preservation). Issue: `Σ output_amounts == Contract.reserve_amount`. Redeem is a flex-transfer to an issuer-owned address (§9.6) with strict conservation — the issuer handles off-chain payout and may spend the received tokens however they choose (including flex-transferring to a "burn" address, but that is an issuer-internal operation outside the BNTP covenant scope). Prior v2 drafts ambiguously stated "redeem allows burn" in this rule — that wording is obsolete per decision #16.
...
optionalData    variable    byte-exact preserved; SIZE ≤ 4096b (Step C Minor #1 fix — §9.2 output reconstruction cost bounded)
```
...
scriptPubKey = OP_FALSE OP_RETURN
               ...
               <thisOutpoint 36b>    UTXO being refreshed (or Contract's spending outpoint at issue per §9.9 rule 8)
...
Protocol-level floor: `royalty ≥ 1` (anti-dust). Protocol-level ceiling: `royalty ≤ my_amount − 1` (Step C Minor #4 clarification — implied by rule "both outputs must satisfy anti-dust amount ≥ 1", but now stated explicitly). A refresh of a `my_amount == 1` UTXO is therefore impossible via royalty payment; such a UTXO must be merged via flex-transfer with a larger UTXO first, or confiscated, or spent directly as a redeem flex-transfer.
...
Script (normative — Step C Moderate #3 option B fix):

1. `HASH160(mpkh_preimage) == expected_hash` → EQUALVERIFY
2. Parse m and n from preimage
3. **On-chain MPKH bounds check (MINIMUM):** `m ≥ 1` → OP_VERIFY. Catches deployer misconfig `m = 0` which would make `OP_CHECKMULTISIGVERIFY` a no-op (silent auth bypass). Additional on-chain checks (`m ≤ n`, `n ≤ 5`, pairwise-distinct pubkeys) are RECOMMENDED but OPTIONAL — enforced off-chain by SDK deploy tooling per §14. Rationale for minimum-on-chain: the `m = 0` attack is critical (no signatures required to spend); duplicate pubkeys and `n > 5` are hygiene (self-harm to deployer, not bypass). Keeps on-chain body cost minimal (~3-4b per MPKH branch) while closing the critical attack class.
4. `OP_CHECKMULTISIGVERIFY` against preimage-derived pubkey set.

**SDK responsibility (normative):** deploy tool MUST reject MPKH preimages where:

- `m < 1` OR `m > n` OR `n > 5`
- Pubkeys are not pairwise-distinct
- Push-prefix byte is not `0x21` (33-byte compressed pubkey push)
- Any pubkey is not a valid secp256k1 compressed pubkey (33 bytes, first byte ∈ {0x02, 0x03})

If the SDK ships a malformed MPKH preimage despite these checks, the resulting UTXO may be deployed but will either be trivially spendable (m=0 case — caught on-chain by rule 3 above) or ineffectively secure (duplicate-pubkey case — spendable by a single key holder). SDK hygiene is the primary defense against hygiene-class attacks; on-chain rule 3 is the defense-in-depth against the critical `m=0` class.
...
- `owner`: 20 raw bytes (PKH) OR 35-171 raw bytes (MPKH preimage); pushed as a single data push. Locking script MUST verify that the owner byte length is **consistent with the tail's `authorityFlags` bit 5 (Step C Moderate #5 fix)** per this invariant: `OP_SIZE(owner) == 20` iff `bit 5 == 0` (PKH); `OP_SIZE(owner) ∈ [35, 171]` iff `bit 5 == 1` (MPKH). Mismatch fails `OP_VERIFY`. Prevents divergent-implementation DoS where a malformed UTXO has push-prefix inconsistent with flag semantics (receiver's wallet might parse by flag alone and produce a spend attempt the on-chain script rejects — unspendable-but-valid-looking UTXO).
...
2. **Input contiguity invariant (Gap 3 closure):** BNTP inputs occupy positions `[0 .. N-1]` in `all_input_outpoints`; optional funding input sits at position `N` (last). This is enforced by `HASH256(all_input_outpoints ‖ funding_outpoint) == hashPrevouts` — the concatenation order binds the layout to consensus-verified `hashPrevouts`. SDK MUST construct txs with this input ordering; wallets MUST reject incoming BNTP txs whose input layout deviates. **No-funding-input case (Step C Moderate #7 fix):** if the tx has no funding input (e.g., all BNTP-only, fees covered by dust margin in outputs), `funding_outpoint` MUST be pushed as a 0-byte empty push; the contiguity invariant degenerates to `HASH256(all_input_outpoints) == hashPrevouts`. SDK validates `|funding_outpoint| ∈ {0, 36}`.
...
8. **Optional change output at index 3 (Gap 4.4 closure, Step C Moderate #6 wording fix).** If the unlocking pushes `change_script_bytes` as a non-empty byte string, the locking script serializes the change output `(change_satoshis 8b LE ‖ varint(len) ‖ change_script_bytes)` and appends to the hashOutputs accumulator. **Covenant-level enforcement is limited to hashOutputs byte-exact match — the covenant does NOT verify that `change_satoshis` equals `funding_input_satoshis − fees`.** `change_satoshis` is owner-controlled (pushed via unlocking). Consequence: in a single-signer refresh the owner cannot steal from themselves; but in **multi-party txs** (e.g., a hypothetical external protocol that pairs refresh with a counterparty-funded input), an adversarial owner could siphon the counterparty's funding via fee manipulation. SDK builders MUST compute `change_satoshis = funding_input_satoshis − fees − dust` honestly; external-protocol authors MUST treat refresh as single-party-funded or add a co-signed fee commitment outside the covenant. If `change_script_bytes` is empty (or missing), no change output is appended.
...
3. Exactly 1 Normal output with:
   ...
   - **new_depth = my_depth (PRESERVED — Step C Critical #1 fix)**. Confiscate is a pure ownership change; does NOT reset depth. New owner may subsequently call `refresh` via issuer attestation to reset depth if desired. Symmetric with §9.4 freeze. Rationale: see §4.2 rule 4.
   - All other tail fields preserved (tokenId, issuerPkh, authorityFlags, freezeAuthHash, confiscAuthHash, optionalData)
4. hashOutputs match
...
Confiscation from Frozen. Same as Normal confiscate but source is Frozen. Unlocking pushes `normal_body_bytes` identical to §9.7 (target is Normal template); **`new_depth = my_depth` (preserved per Step C Critical #1 / decision #39)**. Prior draft said `new_depth = 0` — that wording is obsolete, depth is now preserved symmetric with Normal path 4 confiscate, §4.2 rule 4.
...
```
[N (1 byte, ScriptNum, 1..4)]  ← explicit output count (decision #48)
...
2. **N output count bound (decision #48):** `N ∈ [1, 4]`. Issuer constructs one-shot mint with N ≤ 4 Normal outputs. For larger fan-outs, issuer can deploy multiple Contract UTXOs or perform subsequent flex-transfer splits from the initial Normal outputs. Pragmatic cap for body tractability (output reconstruction is unrolled × 4 mirroring Normal path 1).
3. All N outputs reconstructed using Contract's **inlined** Normal body constant (per §5.5.2). Each candidate output = `owner_push[i] ‖ action_data(0x00) ‖ OP_2DROP ‖ NORMAL_BODY_INLINE ‖ OP_RETURN ‖ reconstructed_tail[i]`.
4. All N outputs have `body_marker = 0x01ff` (first two bytes of `NORMAL_BODY_INLINE`; byte-exact).
5. All N outputs share `tokenId = SHA256(genesisTxId ‖ contractVout ‖ issuerPkh)` where `issuerPkh` is from Contract's tail.
6. Tail fields (issuerPkh, authorityFlags, freezeAuthHash, confiscAuthHash, optionalData) match Contract's tail on every output.
7. All N outputs have `new_depth = 0` (fresh).
8. `Σ output_amounts == Contract.reserve_amount` (amount conservation — Contract's `amount` tail field set at mint time, enforces total supply).
9. **Null-data attestation at output index N** (after all N Normal outputs) — normative position (Step C Moderate #4 fix). `thisOutpoint` field in the null-data payload MUST equal the **Contract's spending outpoint** (the input being spent by this issue tx, readable from `preimage.outpoint`). This binds the attestation to the specific Contract consumption event. Format otherwise identical to refresh attestation (§7.2). Locking script parses null-data at fixed stack slot via `OP_DEPTH` offset relative to unlocking pushes; SDK MUST emit at index N. Resolves §15 open question #2.
...
attestation_depth  2 bytes   ← MUST be 0x0000 at mint (Step C Minor #3); issue path does not read or copy this field to Normal outputs (issue-path outputs have new_depth=0 explicitly per §9.9 rule 6)
...
| Template | PREFIX (shared) | SUFFIX (path-specific) | Inlined Normal body | Body marker | **Body total (measured)** | Gate (revised post-Phase-1A)           | Verdict                         |
| -------- | --------------- | ---------------------- | ------------------- | ----------- | ------------------------- | -------------------------------------- | ------------------------------- |
| Contract | 736b            | ~611b                  | 2624b               | 5b          | **3971b**                 | PASS ≤4100 / PIVOT ≤4300 / ABORT >4300 | **PASS** (129b under ceiling)   |
| Normal   | 883b            | 1722b                  | —                   | 5b          | **2620b**                 | PASS ≤2600 / PIVOT ≤2700 / ABORT >2700 | **PIVOT** (accepted per Step C) |
| Frozen   | 745b            | 531b                   | —                   | 5b          | **1282b**                 | PASS ≤1200 / PIVOT ≤1400 / ABORT >1400 | **PIVOT** (118b under ceiling)  |

**Revised Phase 1A gates (decision #49):** spec §11.1's pre-audit projections (Contract ~3100b, Normal ~2500b, Frozen ~700b) were set before Step C adversarial review and A.3 code fixes added ~40-150b per template. Revised gates reflect honest accounting after:

- Explicit PKH CHECKSIGVERIFY (decision #29, +5b per authority × 4 authorities)
- On-chain MPKH m≥1 bounds check (decision #41, +4b per MPKH branch × 5 branches)
- Output owner size check (decision #43, +6b per output reconstruction × 6 locations)
- Confiscate-preserves-depth correction (decision #39, ~0b)
- Shared PREFIX duplicated across 3 templates (~736b each; BSV Script has no jumps, inlining inherent; Phase 1B source-level refactor via exported constants possible, zero body impact).

**Contract** inlines full Normal body per decision #28 (§5.5.2). Inline alone = 2624b of the 3971b total. One-shot UTXO; alternative (unlocking push) would cost N×2620b per issue tx (~11 KB for N=4) vs current 3971b mint + ~500b issue. Inline wins ~7 KB per token lifecycle.

**Frozen** recalibration from ~700b to ~1300b driven by authority identity cost per path (path 3 + path 4 each invoke `authorityIdentityAsm` ≈ 110b for PKH + MPKH branches) plus explicit CHECKSIGVERIFY and h_Normal SHA256 verify in both paths.

**Normal** PIVOT is accepted per Step C Critical #1 disposition — the 20b over strict PASS is bought by `#39` confiscate-preserve-depth fix + `#41` MPKH m≥1 + `#43` owner size check, all adversarial-review-ratified.
...
### 13.8 Wallet verification rules (Step C Moderate #8 fix)

On-chain script cannot defend against wallet-level spoofing of issuer identity. This section sets normative wallet rules to close the trust-registry UI attack surface.

1. **Raw-byte registry comparison.** Wallets MUST compare received UTXO's `issuerPkh` against the trusted issuer registry by **raw 20-byte equality** — no truncation, no partial prefix/suffix matching, no fuzzy string compare on Base58Check representations.
2. **Unambiguous display.** Wallets MUST NOT display truncated or Base58Check-encoded `issuerPkh` representations without indicating which representation is authoritative. When UI space is constrained, wallets SHOULD display the full 20-byte hex alongside a user-identifier label from the registry.
3. **Near-collision warnings.** Wallets SHOULD warn on near-collisions to previously-approved issuers (e.g., Hamming distance ≤ 8 bytes, or ≥ 4 leading bytes shared) to mitigate vanity-address spoofing where an attacker registers `0x4a3b1c...9e1f` alongside a legitimate `0x4a3b1c...5f9e`.
4. **Registry integrity.** Issuer registry source MUST be authenticated (HTTPS with cert pinning, signed manifest, or equivalent). Wallet MUST reject registry entries lacking integrity attestation.
5. **Depth-based freshness policy.** Wallets SHOULD define a numeric freshness threshold (e.g., `accept iff depth ≤ 32`) and expose it in UI. UTXOs above the threshold prompt user decision: (a) spend as-is (assume issuer honest), (b) refresh via issuer first, (c) reject.
6. **Confiscate-preserve-depth implication (Step C Critical #1 consequence).** After the decision to preserve depth on confiscate (§4.2 rule 4), `depth = 0` now unambiguously implies issuer attestation (issue or refresh). Wallets MAY use `depth = 0` as a trust-level indicator without risk of confiscAuth-forgery ambiguity. Prior-draft behavior (depth reset on confiscate) is rejected.

These rules are not enforced on-chain; they are normative for wallet vendors and SDK implementers providing wallet integration helpers.
...
- **Template hash verification tool (Gap 6 closure):** SDK CLI/lib that (a) compiles each template's body from source, (b) computes `SHA256(body_X)` deployment-time hash, (c) verifies embedded cross-template constants (`h_Frozen` in Normal, `h_Normal` in Contract, `h_Normal` in Frozen) match the actually-compiled bodies, (d) emits a machine-readable manifest for publication. Prevents accidental drift between template sources and the 32-byte constants baked into other templates (mis-matched hashes → broken freeze/issue/unfreeze paths at runtime). Tool runs in CI and is pre-publication gate for any template change. **Additional guards (Step C minors):** tool MUST (i) reject template bodies where any candidate output may serialize to > 65535 bytes (Step C Minor #5; invalidates FD-only varint optimization per decision #38), (ii) verify MPKH preimages in any pre-baked MPKH hash manifest pass full hygiene checks per §8.2 SDK responsibility, (iii) validate Contract tail's `attestation_depth == 0x0000` (Step C Minor #3), (iv) validate `optionalData` ≤ 4096b (Step C Minor #1).
...
| 39  | Confiscate preserves depth                               | **Step C Critical #1 fix.** Confiscate is pure ownership change; does NOT reset `attestation_depth`. New owner inherits `my_depth`; may refresh via issuer if they want freshness reset. Symmetric with freeze/unfreeze. Closes confiscAuth-compromise silent-forgery attack. ~0b body impact. §4.2 rule 4, §9.5 rule 3, §13.8 rule 6.                                                                                                                                                                   |
| 40  | Strict conservation on every path (no burn)              | **Step C Moderate #2 fix.** Stale "redeem allows burn" wording (§4.2 rule 11) removed. All spend paths enforce strict conservation; redeem is a flex-transfer to issuer-owned address with full conservation per §9.6. 0b body. §4.2 rule 11.                                                                                                                                                                                                                                                            |
| 41  | MPKH minimum on-chain bounds (m ≥ 1)                     | **Step C Moderate #3 option B fix.** Script MUST enforce `m ≥ 1` on every MPKH branch (~3-4b per MPKH invocation). Catches deployer-misconfig `m = 0` silent-auth-bypass. Hygiene (`m ≤ n`, `n ≤ 5`, pairwise-distinct pubkeys) is SDK responsibility per §8.2 + §14. ~12-16b total body. §8.2.                                                                                                                                                                                                          |
| 42  | Issue-path `thisOutpoint` = Contract outpoint            | **Step C Moderate #4 fix.** Null-data attestation at issue commits `thisOutpoint = Contract's spending outpoint (from preimage.outpoint)`. Resolves §15 open question #2. §7.2, §9.9 rule 8.                                                                                                                                                                                                                                                                                                             |
| 43  | Output-tuple owner size ↔ flag bit 5                     | **Step C Moderate #5 fix.** Locking script verifies `OP_SIZE(output.owner) == 20 ⟺ flag bit 5 == 0`; `OP_SIZE(output.owner) ∈ [35, 171] ⟺ flag bit 5 == 1`. Mismatch → OP_VERIFY fail. +~25b body (path 1 ×4 iter + path 4 ×1). Prevents divergent-impl DoS. §9.2.                                                                                                                                                                                                                                       |
| 44  | `change_satoshis` is owner-controlled (clarified)        | **Step C Moderate #6 fix.** Covenant only enforces hashOutputs byte-exact; does NOT verify `change_satoshis` derivation. SDK MUST compute honestly; external-protocol authors MUST not rely on covenant for fee integrity in multi-party txs. 0b body. §9.3 rule 8.                                                                                                                                                                                                                                      |
| 45  | `funding_outpoint` optional (empty push when no funding) | **Step C Moderate #7 fix.** If tx has no funding input, push empty byte string for `funding_outpoint`; contiguity invariant degenerates to `HASH256(all_input_outpoints) == hashPrevouts`. SDK validates `                                                                                                                                                                                                                                                                                               | funding_outpoint | ∈ {0, 36}`. 0b body. §9.2. |
| 46  | Wallet verification normative rules                      | **Step C Moderate #8 fix.** New §13.8 with 6 wallet-level rules: raw-byte registry compare, unambiguous display, near-collision warnings, registry integrity, depth-based freshness UI, confiscate-preserves-depth implication. Wallet-vendor-normative, 0b body. §13.8.                                                                                                                                                                                                                                 |
| 47  | Minor editorial / hygiene closures                       | **Step C Minors #1-#5:** optionalData size cap 4096b normative §5.2; cross-refs to pseudo-ASM companion doc disclaimed in front matter; Contract `attestation_depth` MUST be 0 at mint §9.9.1; refresh royalty ceiling `≤ my_amount − 1` explicit §7.4; SDK deploy tool rejects bodies > 65535b §14. 0b body.                                                                                                                                                                                            |
| 48  | Issue-path `N ≤ 4` bound                                 | **Phase 1A.3 ratified.** Contract path 6 issue produces `N ∈ [1, 4]` Normal outputs, mirroring Normal flex-transfer's M bound. Larger fan-outs via multiple Contract UTXOs or subsequent flex-transfer splits. Kept unrolled × 4 for body tractability. §9.9 rule 2.                                                                                                                                                                                                                                     |
| 49  | Phase 1A body gate revision (honest post-measurement)    | **Phase 1A.3 ratified.** Revised gates: Contract PASS ≤4100 / PIVOT ≤4300 / ABORT >4300; Frozen PASS ≤1200 / PIVOT ≤1400 / ABORT >1400; Normal gate unchanged (PASS ≤2600 / PIVOT ≤2700 / ABORT >2700). Measured: Contract 3971b PASS, Normal 2620b PIVOT (Step C disposition), Frozen 1282b PIVOT. Spec §3 and §11.1 updated with measurements.                                                                                                                                                         |
| 50  | §9.8 confiscate-from-frozen depth semantics              | **Phase 1A.3 editorial fix.** §9.8 prose said "new_depth = 0" — stale after Step C #39 which mandated preserve depth on all confiscate paths. Corrected to "new_depth = my_depth" to match decision #39 and §4.2 rule 4. §9.8.                                                                                                                                                                                                                                                                           |
| 51  | Phase 1B PREFIX source-refactor (scheduled)              | **Not applied in Phase 1A.** Shared PREFIX blocks (`COVENANT_*_ASM`, `SIGHASH_CHECK_ASM`, `PREIMAGE_PARSE_ASM`, `TAIL_CACHE_ASM`, `authorityIdentityAsm`) are duplicated verbatim in `normal-body.ts`, `contract-body.ts`, `frozen-body.ts` (~736b each × 3 = ~2200b source-level dup). BSV Script has no jumps — on-chain dup is inherent. Phase 1B exports these constants from a shared module, eliminating source-level dup. Zero body impact, improves maintainability + prevents accidental drift. |
...
3. ~~**OptionalData size limit** — reuse v1 limit (4096 bytes)?~~ **Closed (Step C Minor #1):** 4096-byte cap now normative in §5.2 tail layout.
...
- **2026-04-18** — **Phase 1A.3 Contract + Frozen templates measured** (parallel opus agent round). Decisions #48-#51 ratified:
  - **Contract: PASS 3971b** (§9.9 path 6 issue; inlines Normal body 2624b + shared PREFIX 736b + path-6 logic ~611b; one-shot mint UTXO amortized over token lifecycle). Gate revised (decision #49).
  - **Frozen: PIVOT 1282b** (§§9.7 unfreeze + 9.8 confiscate-from-frozen; shared PREFIX 745b + path-3 260b + path-4 271b + misc). Spec §11.1 pre-audit 700b projection stale; revised to ~1300b realistic.
  - **Decision #48:** Contract issue path N ≤ 4 (ratified from pragmatic cap; mirrors Normal M bound).
  - **Decision #50:** §9.8 prose fix — "new_depth = 0" → "new_depth = my_depth" per Step C Critical #1 / decision #39 (preserve depth on all confiscate paths, including Frozen→Normal).
  - **Decision #51:** shared PREFIX source-refactor scheduled for Phase 1B (export from shared module; duplicated inline per BSV Script no-jump semantic, but source-level dup ~2200b across 3 templates is ripe for cleanup).
  - All three Phase 1A templates now byte-measured. Full per-UTXO cost matrix updated in §11.1.
...
- **2026-04-18** — **Step C adversarial spec review** (`BNTP_V2_SPEC_ADVERSARIAL_REVIEW.md`, delegated opus agent): SHIP-WITH-FIXES verdict, 1 critical + 6 moderate + 5 minor findings. 22 attack surfaces probed clean (conservation, tokenId enforcement, attestation binding, cross-template SHA-256 PRS, depth saturation, FORKID replay, tokenId collision, output-count bounds, N≤32, contiguity, anti-dust). User ratified all findings; all applied (decisions #39-#47, §15):
  ...
  - **#41:** on-chain `m ≥ 1` MPKH bounds check (option B — minimal on-chain + SDK hygiene). +~12-16b body.
  - **#42:** issue-path `thisOutpoint = Contract spending outpoint` normative.
  - **#43:** output-tuple owner `OP_SIZE` ↔ flag bit 5 consistency check. +~25b body.
  - **#44-#45:** `change_satoshis` owner-controlled clarification; `funding_outpoint` empty-push handling.
  - **#46:** new §13.8 wallet verification normative rules (trust registry compare, display, near-collision, freshness policy).
  - **#47 minors:** optionalData ≤ 4096b normative §5.2; pseudo-ASM cross-refs disclaimed; Contract `attestation_depth == 0` normative; royalty ceiling explicit; SDK deploy tool guards.
  - Projected body post-Step C: 2574 + ~40-50b ≈ **2619b PIVOT** (20b over strict G5 PASS, 81b under ABORT). Real-ASM measurement pending A.3 agent round.
